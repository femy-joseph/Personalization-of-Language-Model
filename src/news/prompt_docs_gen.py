...
with open("/content/sample_data/Lamp4-copy.json","r") as file:
    data=json.load(file)
    ...
    for i in range(len(data)):
        ...
        #to fetch qid from the query
        dic = query_df.to_dict()
        qid = (dic['qid'][0])
        #Processing to fetch the prompted doc title and text
        df = pd.merge(user_df, prompt_docs, on='docno')
        df = df.drop(df.columns[-2:],axis=1)
        first_prompt_doc_text = df.to_dict('records')[0]['text_x']
        first_prompt_doc_title = df.to_dict('records')[0]['title_x']
        original_query = query_df.to_dict('records')[0]['query']
        input_to_LLM = original_query + 'here is the example article query pair ' + first_prompt_doc_text + first_prompt_doc_title
        #print(input_to_LLM)
